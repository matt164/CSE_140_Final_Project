...
int main(int argc, char **argv)
{
	...
	{
	    total_clock_cycles++;
	    cout << "\ntotal_clock_cycles: " << std::dec << total_clock_cycles << endl;
		...
		for(int i = 0; i < pipeline.size(); i++)          //for all of the instructions in the pipeline perform the appropriate stage
		{
            ...
            else if(skip_next == 1)            //if we are stalling we can skip the next instruction as it would just be a NOP as well
            {
                skip_next = 0;
                break;
            }
...
void pipeline_control(instruction &inst)       //control function that calls the appropriate function for an instruction
{
    ...
    {
        cout << "Data hazard detected (NOP " << NOP_out << ")\n";         //tells what NOP you are on for output
		inst.stall--;
		skip_next = 1;                                                //skips the execution of the next instruction so you don't need to waste time to just skip it too
		if(inst.stall > 0)
            NOP_out++;                                      //prints NOP 1 or NOP2 depending on what cycle of the stall its on
    }
}
...
void decode(instruction &instr)       //decode function
{
    ...
    prev_dest_2 = prev_dest;                           //assigns the last instruction to be the second to last and updates the last instruction with the data of this instruction
    prev_dest = -1;                                    //defaults to -1 as long as the instruction doesn't modify a register
    
	if(instr.op_code == 0)  //if it is an r type instruction
		prev_dest = instr.rd;   //set the last destination to rd
	if(instr.op_code == 35)     //if it is load word
        prev_dest = instr.rt;       //set the last destination to rt
